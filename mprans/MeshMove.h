...
namespace proteus
{
  ...
  public:
    ...
    inline void evaluateCoefficients(const double* materialProperties,
				     double* strain,
				     double* stress,
				     double* dstress)
    {
	  ...
	  for(int k=0;k<nQuadraturePoints_element;k++)
		...
		*grad_w(&D[2*nSpace]),
	      ...
	      for (int j=0;j<nDOF_trial_element;j++)
		{
	      ...
	      evaluateCoefficients(&materialProperties[materialTypes[eN]*nMaterialProperties],
				   strain,
				   stress,
				   dstress);
	      //
