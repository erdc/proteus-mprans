#ifndef VolumeAveragedVOF_H
#define VolumeAveragedVOF_H
#include <cmath>
#include <iostream>
...
namespace proteus
{
  class VolumeAveragedVOF_base
  {
  public:
    virtual ~VolumeAveragedVOF_base(){}
  ...
  class VolumeAveragedVOF : public VolumeAveragedVOF_base
  {
  public:
	...
	{ 
      ...
      std::cout<<"VolumeAveragedVOF global conservation============================================================="<<globalConservation<<std::endl;
    }
	  ...
	  for  (int kb=0;kb<nQuadraturePoints_elementBoundary;kb++) 
	      ...
	      for (int j=0;j<nDOF_trial_element;j++)
		{
	      ...
	      exteriorNumericalAdvectiveFluxDerivative(isDOFBoundary_u[ebNE_kb],
						       isFluxBoundary_u[ebNE_kb],
						       normal,
						       df_ext,//VRANS holds porosity &ebqe_velocity_ext[ebNE_kb_nSpace],
						       dflux_u_u_ext);
	      //DoNothing for now
	      //
  ...
  inline VolumeAveragedVOF_base* newVolumeAveragedVOF(int nSpaceIn,
				...
				int CompKernelFlag)
  {
    return proteus::chooseAndAllocateDiscretization<VolumeAveragedVOF_base,VolumeAveragedVOF,CompKernel>(nSpaceIn,
