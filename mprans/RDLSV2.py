...
class OneLevelRDLSV2(OneLevelTransport):
    ...
    def __init__(self,
        ...
        if debugRDLS:
            ...
            self.q[('w*dV_H',0)] =  numpy.zeros((self.mesh.nElements_global,self.nQuadraturePoints_element,self.nDOF_trial_element[0]),'d')
            self.q[('w*dV_m',0)] =  self.q[('w*dV_H',0)]
            self.q[('grad(w)*dV_H',0)] =  numpy.zeros((self.mesh.nElements_global,self.nQuadraturePoints_element,self.nDOF_trial_element[0],self.nSpace_global),'d')
            ...
            self.ebqe[('w*dS_H',0)] = numpy.zeros((self.mesh.nExteriorElementBoundaries_global,self.nElementBoundaryQuadraturePoints_elementBoundary,self.nDOF_trial_element[0]),'d')
            ...
            self.q[('gradNorm',0,0)] =  numpy.zeros((self.mesh.nElements_global,self.nQuadraturePoints_element),'d')
        ...
        #mwf just for debugging
        self.q_tau_last = numpy.zeros(self.q[('u',0)].shape,'d')
        self.q[('r',0)] = numpy.zeros(self.q[('u',0)].shape,'d')
        self.q[('subgridError',0)] = numpy.zeros(self.q[('u',0)].shape,'d')
        self.q[('Lstar*w*dV',0,0)] = numpy.zeros(self.q[('w',0)].shape,'d')
    ...
    def getResidual(self,u,r):
        ...
        self.getResidualNew(u,self.globalResidualDummy)
        if numpy.linalg.norm(self.globalResidualDummy-r) > 1.0e-4:
            pdb.set_trace()
    def getResidualNew(self,u,r):
        import pdb
        import copy
        #try to use 1d,2d,3d specific modules
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cResidual = cRDLSV2.calculateResidual
            if self.nSpace_global == 2:
                cResidual = cRDLS2D.calculateResidual
            elif self.nSpace_global == 1:
                cResidual = cRDLS1D.calculateResidual
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cResidual = cRDLSQ.calculateResidual
            if self.nSpace_global == 2:
                cResidual = cRDLS2DQ.calculateResidual
            elif self.nSpace_global == 1:
                cResidual = cRDLS1DQ.calculateResidual
        #mwf debug
        #pdb.set_trace()
        r.fill(0.0)
        ...
        cResidual(#element
            ...
            self.mesh.nElements_global,
                  ...
                  self.q[('mt',0)],
                  self.q[('r',0)],
                  self.q[('subgridError',0)],
                  self.q[('Lstar*w*dV',0,0)],
                  self.q_tau_last,
                  #mwf end debugging
	...
	cfemIntegrals.zeroJacobian_CSR(self.nNonzerosInJacobian,
        ...
        #try to use 1d,2d,3d specific modules
        if isinstance(self.u[0].femSpace,C0_AffineLinearOnSimplexWithNodalBasis):
            cJacobian = cRDLSV2.calculateJacobian
            if self.nSpace_global == 2:
                cJacobian = cRDLS2D.calculateJacobian
            elif self.nSpace_global == 1:
                cJacobian = cRDLS1D.calculateJacobian
        elif isinstance(self.u[0].femSpace,C0_AffineQuadraticOnSimplexWithNodalBasis):
            cJacobian = cRDLSQ.calculateJacobian
            if self.nSpace_global == 2:
                cJacobian = cRDLS2DQ.calculateJacobian
            elif self.nSpace_global == 1:
                cJacobian = cRDLS1DQ.calculateJacobian
        #cRDLS.calculateJacobian(self.mesh.nElements_global,
        cJacobian(#element
            self.u[0].femSpace.elementMaps.psi,
            self.u[0].femSpace.elementMaps.grad_psi,
            self.mesh.nodeArray,
	...
	#assume all components live on the same mesh
        ...
        self.testSpace[0].getBasisGradientValues(self.elementQuadraturePoints,
                                                  self.q['inverse(J)'],
                                                  self.q[('grad(w)',0)])
        cfemIntegrals.calculateWeightedShapeGradients(self.elementQuadratureWeights[('H',0)],
                                                      self.q['abs(det(J))'],
                                                      self.q[('grad(w)',0)],
                                                      self.q[('grad(w)*dV_H',0)])
	...
	#assume all components live on the same mesh
        ...
        cfemIntegrals.calculateWeightedShapeGlobalExteriorTrace(self.mesh.exteriorElementBoundariesArray,
                                                                self.mesh.elementBoundaryElementsArray,
                                                                self.mesh.elementBoundaryLocalElementBoundariesArray,
                                                                self.elementBoundaryQuadratureWeights[('H',0)],
                                                                self.ebqe['sqrt(det(g))'],
                                                                self.ebqe[('w',0)],
                                                                self.ebqe[('w*dS_H',0)])
