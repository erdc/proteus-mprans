import proteus
from proteus.mprans.cRANS3PSed import *
...
class Coefficients(proteus.TransportCoefficients.TC_base):
    ...
    def __init__(self,
        ...
        self.stokes=stokes
        self.RANS_model_ID=RANS_model_ID
        ...
        if nd==2:
            ...
            self.vectorName="sedimentVelocity"

    def attachModels(self,modelList):
        #level set
        self.model = modelList[0]
        if self.RANS_model_ID != None:
            self.RANS_model = modelList[self.RANS_model_ID]
            self.p_dof = self.RANS_model.u[0].dof
            self.q_fluidVelocity = self.RANS_model.q[('velocity',0)]
        else:
            self.p_dof = self.model.u[0].dof.copy()
            self.p_dof[:] = 1.0
            self.q_fluidVelocity = self.model.q[('velocity',0)].copy()
            self.q_fluidVelocity[:] = 0.0
...
class LevelModel(proteus.Transport.OneLevelTransport):
    ...
    def __init__(self,
        ...
        if self.postProcessing:
            from proteus import RANS3PSed as oldRANS3PSed
            self.tmpvt = oldRANS3PSed.OneLevelRANS3PSed(uDict,
                 phiDict,
                 testSpaceDict,
                 matType,
                 dofBoundaryConditionsDict,
...
#                assert(self.nElementBoundaryQuadraturePoints_elementBoundary == 1)
        ...
        self.q['sedimentForce'] = numpy.zeros((self.mesh.nElements_global,self.nQuadraturePoints_element,self.nSpace_global),'d')
	...
	for ci in range(1,self.nc):
        ...
        self.rans2p = cRANS3PSed_base(self.nSpace_global,
    ...
    def getResidual(self,u,r):
        ...
        self.rans2p.calculateResidual(#element
            ...
            self.u[0].dof,
            self.coefficients.p_dof,
            self.u[1].dof,
            self.u[2].dof,
            self.u[3].dof,
            self.coefficients.g,
            self.q['sedimentForce'],
    ...
    def calculateAuxiliaryQuantitiesAfterStep(self):
        if self.postProcessing:
            from proteus.cRANS3PSed import calculateVelocityAverage as cva
